    ...
    for (ll t_id = 1; t_id <= t; t_id++) {
        ...
        if (type == 1) {  // max_k(m) assume Theta(sqrt(n)) valori distinti
            ...
            for (int i = 0; i < k; ++i) {
                int mid = 30 * m * (i + 1);
                for (int j = 0; j < m; ++j) {
                    l[i * k + j] = rnd.next(mid - 10 * m, mid);
                    r[i * k + j] = rnd.next(mid, mid + 10 * m);
                }
            }
        } else if (type == 5) {
            ll maxc = atoi(argv[5]);
            ...
            while (bsl != bsu) {
                ll bsm = (bsl + bsu) / 2;
                if (bsm * (log(bsm) - 0.577) >= n) bsu = bsm;
                else bsl = bsm + 1;
            }

            ll k = bsl, i = 0;
            for (ll group = 1;; group++) {
                if (i == n) break;
                ll cnt_new = (k + group / 2) / group;
                cnt_new = min(cnt_new, n - i);
                if (cnt_new == 0) cnt_new = 1;
                ll mid = 30 * (i + cnt_new);
                for (ll j = 0; j < cnt_new; j++) {
                    l[i + j] = rnd.next(mid - 10 * t, mid);
                    r[i + j] = rnd.next(mid, mid + 10 * t);
                }
                i += cnt_new;
            }
        }
